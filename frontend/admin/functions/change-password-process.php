...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $confirm_password = isset($_POST['confirm_new_password']) ? trim($_POST['confirm_new_password']) : '';

    // Basic validation
    if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
        header("Location: /Kapelicious/frontend/admin/pages/change-password.php?error=empty_fields");
        exit;
    }

    if ($new_password !== $confirm_password) {
        header("Location: /Kapelicious/frontend/admin/pages/change-password.php?error=password_mismatch");
        exit;
    }
    ...
    if (strlen($new_password) < 8) {
        header("Location: /Kapelicious/frontend/admin/pages/change-password.php?error=weak_password");
        exit;
    }
    ...
    if ($result_message === "Password updated successfully.") {
        header("Location: /Kapelicious/frontend/admin/pages/change-password.php?success=1");
        exit;
    } else {
        // For security, you might want to pass error codes instead of raw messages
        // Adjust as needed
        header("Location: /Kapelicious/frontend/admin/pages/change-password.php?error=" . urlencode($result_message));
        exit;
    }
} else {
    // If accessed without POST data, redirect back to the change password form
    header("Location: /Kapelicious/frontend/admin/pages/change-password.php");
    exit;
}
?>